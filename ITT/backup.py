...
def ocrToStr(fullPath, outTxtPath, fileName, lang='eng'): #디폴트는 영어로 추출
 ...
 # 3    Default, based on what is available.
    ...
    while step < 2.5:
       outText = image_to_string(img_result, lang=lang, config='--psm 3 --oem 2 -c preserve_interword_spaces=1')
       if outText is '':
          img_result = cv2.resize(img_source, None, fx=step, fy=step, interpolation=cv2.INTER_CUBIC)
          img_result = cv2.cvtColor(img_result, cv2.COLOR_BGR2GRAY)
